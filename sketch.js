...
function setup() {
    ...
    // 實例化選單
    let menuItems = ["第一單元作品", "第一單元講義", "測驗區", "回到首頁", "淡江大學"];
...
class SideMenu {
    ...
    // 檢查滑鼠是否在觸發區域
    checkHover() {
        // 如果滑鼠在左側觸發區 OR 滑鼠在選單上方 (currentX + w)
        if (mouseX < this.hover_area || mouseX < this.currentX + this.w) {
    ...
    display() {
        ...
        for (let i = 0; i < this.items.length; i++) {
            let itemY = i * this.h_item + 80; // ***** 修改：起始 Y 座標調整為 80 *****
            ...
            // 繪製選項文字
            text(this.items[i], 20, itemY + this.h_item / 2);
        }
        ...
        if (this.currentX > this.x_hidden + 1) { 
            for (let i = 0; i < this.items.length; i++) {
                if (this.checkItemHover(i)) {
                    ...
                    if (i === 0) { // 第一單元作品
                        iframe.src = 'https://diegohsieh0609-art.github.io/20251020/';
                        container.style.display = 'block';
                        overlay.style.display = 'block';
                    } else if (i === 1) { // 第一單元講義
                        iframe.src = 'https://hackmd.io/@bnL4ivPZRWiV3KHVjCq51Q/S17n8QCixg';
                        container.style.display = 'block';
                        overlay.style.display = 'block';
                    } else if (i === 2) { // 測驗區
                        iframe.src = 'https://diegohsieh0609-art.github.io/20251103/';
                        container.style.display = 'block';
                        overlay.style.display = 'block';
                    } else if (i === 4) { // 淡江大學
